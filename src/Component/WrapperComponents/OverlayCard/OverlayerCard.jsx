...
export default function MultiActionAreaCard({ movie }) {
  ...
  const addMovieToWatchListHandler = () => {
    ...
    if (movies.length >= 5) {
      ...
      dispatch(addMovieToWatchList(movieToAdd));  // Add movie to watchlist if below limit
    }
  };
  ...
  return (
    <>
      <Card className="overlay-card-content" sx={{ maxWidth: 'fitcontent', backgroundColor: '#16181f' }}>
        <CardActionArea>
          <CardMedia
            component="img"
            height={150}
          ...
          />
          <CardActions>
            <Button onClick={handleOpen} fullWidth variant="contained" sx={{ backgroundColor: '#fff', color: '#16181f' }}>
              <PlayArrowIcon /> {isLogIn ? 'Watch Now' : 'Login Now'}
            </Button>
            <Button small variant="outlined" sx={{ color: '#fff' }} onClick={addMovieToWatchListHandler} disabled={!isLogIn}>
              <AddIcon />
            </Button> 
          </CardActions>
          <CardContent>
